...
export default class LekhakNewsletterComponent extends BaseComponent {
    async onInit() {
        registerNavHandlers();
        setPageMeta('Newsletters', 'Manage email campaigns and subscribers');
    }
    ...
    render() {
        ...
        <div style="padding: 2.5rem 2rem; max-width: 1200px; margin: 0 auto; font-family: 'Inter', sans-serif;">
            <header style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: flex-end;">
                <div>
                    <h1 style="font-family: 'Outfit', sans-serif; font-size: 2.2rem; margin: 0; color: var(--text);">
                        <span style="font-size: 2.5rem; margin-right: 10px; vertical-align: middle;">✉️</span>
                        Newsletters
                    </h1>
                    <p style="color: var(--text-dim); margin-top: 5px;">Manage email campaigns and subscribers</p>
                </div>
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 2rem;">
                <div style="background:var(--bg); border:1px solid var(--border); padding:1rem; border-radius:8px;">Subscribers: 5,600</div><div style="background:var(--bg); border:1px solid var(--border); padding:1rem; border-radius:8px;">Open Rate: 24%</div>
            </div>
            ...
            <div style="background:var(--glass-bg,#fff); border:1px solid var(--border); border-radius:12px; padding:3rem; text-align:center; color:var(--text-dim);">
                ...
                <p>The Newsletters engine is active. Configuration and data views will appear here.</p>
            </div>
        </div>
        `;
    }
