...
export default class LekhakGdprComponent extends BaseComponent {
    async onInit() {
        registerNavHandlers();
        setPageMeta('GDPR', 'Manage cookie consent');
    }
        ...
        <div style="padding: 2.5rem 2rem; max-width: 1200px; margin: 0 auto; font-family: 'Inter', sans-serif;">
            <header style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: flex-end;">
                <div>
                    ...
                    <p style="color: var(--text-dim); margin-top: 5px;">Manage cookie consent</p>
                </div>
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 2rem;">
                <div style="background:var(--bg); border:1px solid var(--border); padding:1rem; border-radius:8px;">Consent Given: 45,000</div><div style="background:var(--bg); border:1px solid var(--border); padding:1rem; border-radius:8px;">Rejected: 1,200</div>
            </div>
            ...
            <div style="background:var(--glass-bg,#fff); border:1px solid var(--border); border-radius:12px; padding:3rem; text-align:center; color:var(--text-dim);">
                ...
                <p>The GDPR engine is active. Configuration and data views will appear here.</p>
            </div>
        </div>
        `;
    }
