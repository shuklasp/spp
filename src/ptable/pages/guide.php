 ...
 *
 * Generated for application: ptable
 *
 * HOW THIS PAGE WORKS:
 *   - This is a "native PHP page" rendered by the SPP view layer
 *   - It is mapped in etc/apps/ptable/pages.yml as:
 *       guide:
 *         url: pages/guide.php
...
<div style="max-width: 1100px; margin: 2rem auto; padding: 0 1rem; font-family: Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
    <div style="background: #fff; border-radius: 16px; padding: 2.5rem; box-shadow: 0 4px 20px rgba(0,0,0,0.04); border: 1px solid #e2e8f0;">
        ...
        <p style="color: #94a3b8; font-size: 0.95rem; margin-bottom: 2.5rem;">Application: <code style="background:#f1f5f9;padding:2px 8px;border-radius:6px;">ptable</code> &mdash; Everything a complete novice needs to build any workflow</p>
        ...
        <p style="color:#475569; line-height:1.8;">
            ...
            <code>native</code> (raw PHP), <code>api</code> (headless REST), <code>dropin</code> (low-code HTML/PHP). Your app is: <code>ptable</code>.
        </p>
        ...
        <p style="color:#475569; line-height:1.8;">Every SPP app has two main directories: <code>etc/apps/ptable/</code> for configuration, and <code>src/ptable/</code> for source code.</p>
...
└── spp/                           # Framework core (do not modify)
    ...
    └── etc/global-settings.yml    # All apps registered here</pre>
        ...
        <p style="color:#475569; line-height:1.8;">Routes are defined in <code>etc/apps/ptable/pages.yml</code>. SPP supports five route types:</p>

        <h3 style="color:#475569; margin-top:1.5rem;">3a. pages.yml Route Types</h3>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
# ── etc/apps/ptable/pages.yml ──

defaults:
  home: home                              # Default page when no route matches
  pagedir: /src/ptable              # Base directory for page files
...
pages:
  ...
  api/v1/items/{id}:
    controller: \App\ptable\Serv\ApiController@show</pre>
...
// Cache file: var/cache/routes_ptable.php</pre>
        ...
        <p style="color:#475569; line-height:1.8;">Controllers live in <code>src/ptable/serv/</code>. They handle requests and return either rendered Blade HTML or JSON data.</p>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── src/ptable/serv/HomeController.php ──
...
class HomeController
{
    ...
    // run('viewName', $data) renders resources/ptable/views/viewName.blade.php
    public function index()
    {
        $blade = \SPPMod\Drishyam\SPPBlade::getInstance();
        return $blade-&gt;run('home', [
            'title'   =&gt; 'Welcome to ptable',
    ...
    public function saveAndRedirect()
    {
        ...
        <p style="color:#475569; line-height:1.8;">Blade is the template engine (from Laravel, extended by SPP). Templates live in <code>resources/ptable/views/</code>.</p>

        <h3 style="color:#475569; margin-top:1.5rem;">5a. Layout System</h3>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
{{-- ── resources/ptable/views/layout.blade.php ── --}}
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
{{-- ── resources/ptable/views/home.blade.php ── --}}
...
@endsection</pre>
        ...
        <p style="color:#475569; line-height:1.8;">SPP-UX is the reactive component system. Components are JavaScript classes in <code>src/ptable/comp/</code>.</p>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── comp/my-counter.js ──
...
// Themes defined in resources/ptable/themes/ with theme.yml</pre>
...
\SPP\SPPEvent::listen('PageNotFound', [new \App\ptable\Events\ErrorHandler(), 'onNotFound']);
...
events:
  - event: "AppBoot"
    handler: "App\ptable\Events\AppBootHandler::onBoot"
  - event: "PageNotFound"
    handler: "App\ptable\Events\ErrorHandler::onNotFound"
  - event: "core.error.exception"
    handler: "App\ptable\Events\ErrorHandler::onException"</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
# ── etc/apps/ptable/services.yml ──
services:
  - name: "task.create"
    script: "src/ptable/serv/task_create.php"
  - name: "task.list"
    script: "src/ptable/serv/task_list.php"
...
// ── PHP (src/ptable/serv/task_create.php) ──
...
try {
    ...
    return $blade-&gt;run('error', ['message' =&gt; 'Something went wrong']);
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── src/ptable/middleware/AuthGuard.php ──
namespace App\ptable\Middleware;
...
// ── etc/apps/ptable/middleware.yml (global pipeline) ──
middleware:
  global:
    - \App\ptable\Middleware\CorsMiddleware
    - \App\ptable\Middleware\SessionMiddleware
  groups:
    auth:
      - \App\ptable\Middleware\AuthGuard
    api:
      - \App\ptable\Middleware\ApiRateLimit

// Per-route middleware (in pages.yml):
// dashboard:
//   controller: \App\ptable\Serv\DashboardController@index
//   middleware: [auth]
...
// Each middleware calls $next() to pass to the next one.</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── Creating an Entity ──
// Run: php spp.php make:entity --app=ptable Task
// This creates src/ptable/entities/Task.php

namespace App\ptable\Entities;

class Task extends \SPPMod\SPPDB\Entity
{
    protected string $table = 'ptable_tasks';  // Uses table prefix
...
// ── Migrations ──
// php spp.php migrate --app=ptable
// php spp.php migrate:rollback --app=ptable
// php spp.php db:seed --app=ptable</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
# ── etc/apps/ptable/forms/contact.yml ──
id: contact
...
if ($result['success']) {
    header('Location: /ptable/dashboard');
}
...
//   session_save_path: tcp://127.0.0.1:6379</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── Language files: resources/ptable/lang/en.json ──
{
...
Container::bind('mailer', function() {
    return new \App\ptable\Services\MailService();
});

// ── Singleton (same instance every time) ──
Container::singleton('cache', function() {
    return new \App\ptable\Services\CacheService();
});
...
// File naming: test.TaskTest.php in src/ptable/tests/</pre>
...
// Also: -&gt;toBeTrue(), -&gt;toBeFalse(), -&gt;toBeNull(), -&gt;toContain()</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
php spp.php test:run --app=ptable               # Run full test suite
php spp.php test:run --app=ptable --coverage    # With code coverage report
php spp.php test:run --app=ptable TaskTest      # Run single test class
php spp.php test:blueprint --app=ptable          # Auto-generate test stubs
php spp.php test:monkey --app=ptable             # Fuzz/monkey testing
php spp.php test:evolve --app=ptable             # Evolutionary testing</pre>

        <!-- ================================================================ -->
        <!-- SECTION 19: Themes -->
        <!-- ================================================================ -->
        <h2 style="color:#6366f1; margin-top:2.5rem; border-bottom: 2px solid #e2e8f0; padding-bottom: 0.5rem;">19. Themes</h2>
        <p style="color:#475569; line-height:1.8;">Themes are defined in <code>resources/ptable/themes/</code> with a <code>theme.yml</code> config.</p>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
# ── resources/ptable/themes/default/theme.yml ──
name: "Default Theme"
version: "1.0"
css:
...
apps:
  ptable:
    base_url: /ptable
    table_prefix: ptable_
    type: mixed
    shared_group: core
    etc_path: etc/apps/ptable
    src_path: src/ptable

# ── etc/apps/ptable/config.yml ── (app-specific)
app:
  name: "ptable"
...
$value = \SPP\SPPConfig::get('app.name');       // "ptable"
...
// Log rotation is automatic (configurable max file size)</pre>
        ...
        <p style="color:#475569; line-height:1.8;">Follow these steps to build your first feature in <code>ptable</code>:</p>
        ...
        <div style="background:#f0fdf4; border-left:4px solid #22c55e; padding:1.2rem 1.5rem; border-radius:0 10px 10px 0; margin:1rem 0;">
            <ol style="color:#475569; line-height:2.2; margin:0; padding-left:1.2rem;">
                <li><b>Explore your app</b> &mdash; Visit <code>/ptable/home</code> in your browser</li>
                <li><b>Edit a controller</b> &mdash; Modify <code>src/ptable/serv/HomeController.php</code></li>
                <li><b>Edit a Blade view</b> &mdash; Modify <code>resources/ptable/views/home.blade.php</code></li>
                <li><b>Create an entity</b> &mdash; Run <code>php spp.php make:entity --app=ptable Task</code></li>
                <li><b>Run migrations</b> &mdash; Run <code>php spp.php migrate --app=ptable</code></li>
                <li><b>Add a new route</b> &mdash; Edit <code>etc/apps/ptable/pages.yml</code></li>
                <li><b>Create a YAML form</b> &mdash; Add a new file in <code>etc/apps/ptable/forms/</code></li>
                <li><b>Build a component</b> &mdash; Create a JS file in <code>src/ptable/comp/</code></li>
                <li><b>Write tests</b> &mdash; Add tests in <code>src/ptable/tests/</code></li>
                <li><b>Run tests</b> &mdash; <code>php spp.php test:run --app=ptable</code></li>
        ...
        <div style="margin-top:2rem; padding:1.5rem; background:#eff6ff; border-radius:10px; text-align:center;">
            ...
            <p style="color:#64748b; font-size:0.85rem; margin:0.5rem 0 0;">Remove the <code>'guide'</code> route from <code>etc/apps/ptable/pages.yml</code> and delete this file.</p>
        </div>
