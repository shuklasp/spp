            ...
            <div class="mobile-studio fade-in">
                <header class="studio-header glass-panel" style="display:flex; justify-content:space-between; align-items:center; padding: 8px 20px; border-radius: 0; border-bottom: none; border-top: none; border-left: none; border-right: none;">
                    ...
                    <div style="display:flex; gap:10px; align-items:center;">
                        <div class="segmented-control" style="display:flex; background: rgba(0,0,0,0.2); padding: 2px; border-radius: 8px;">
                            <button class="btn ${this.state.breakpoint === 'mobile' ? 'primary-btn' : 'ghost-btn'} btn-sm" style="border:none; padding: 0 10px;" @click=${() => this.setState({breakpoint: 'mobile'})}>📱</button>
                            <button class="btn ${this.state.breakpoint === 'tablet' ? 'primary-btn' : 'ghost-btn'} btn-sm" style="border:none; padding: 0 10px;" @click=${() => this.setState({breakpoint: 'tablet'})}> tablet </button>
                            <button class="btn ${this.state.breakpoint === 'desktop' ? 'primary-btn' : 'ghost-btn'} btn-sm" style="border:none; padding: 0 10px;" @click=${() => this.setState({breakpoint: 'desktop'})}>💻</button>
                        </div>

                        <div class="zoom-controls glass-panel" style="display:flex; align-items:center; gap:10px; padding: 2px 10px; border-radius: 8px; background: rgba(0,0,0,0.2);">
                            <button class="btn-icon btn-xs" @click=${() => this.setState({zoom: Math.max(25, this.state.zoom - 10)})}>-</button>
                            <span style="font-size: 0.65rem; min-width: 35px; text-align: center; font-family: 'JetBrains Mono';">${this.state.zoom}%</span>
                            <button class="btn-icon btn-xs" @click=${() => this.setState({zoom: Math.min(200, this.state.zoom + 10)})}>+</button>
                        </div>
                ...
                <div class="studio-layout">
                    ...
                    <!-- Center: Preview -->
                    <main class="studio-preview" style="border-radius: 0; background: var(--bg-dark); background-image: radial-gradient(rgba(255,255,255,0.05) 1px, transparent 1px); background-size: 30px 30px; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding: 150px 50px; overflow: auto; min-height: 0;">
                ...
                <!-- Pro Status Bar -->
                <footer class="studio-footer" style="height: 32px; background: var(--sidebar-bg); border-top: 1px solid var(--glass-border); display: flex; justify-content: space-between; align-items: center; padding: 0 15px; font-size: 0.65rem; color: var(--text-dim); z-index: 1000;">
                    <div style="display: flex; gap: 20px; align-items: center;">
                        <span style="display: flex; align-items: center; gap: 5px;"><span style="color: var(--success);">●</span> SYSTEM READY</span>
                        <span style="opacity: 0.6;">CONTEXT: <strong style="color: var(--primary-color);">${this.state.activeScreenId.toUpperCase()}</strong></span>
                        <span style="opacity: 0.6;">SELECTED: <strong style="color: var(--text-main);">${this.state.selectedComponentId || 'NONE'}</strong></span>
                    </div>
                    <div style="display: flex; gap: 20px; align-items: center;">
                        <span style="display: flex; align-items: center; gap: 5px;">VIEWPORT: <strong style="color: var(--text-main);">${this.state.breakpoint.toUpperCase()}</strong></span>
                        <span style="display: flex; align-items: center; gap: 5px;">ZOOM: <strong style="color: var(--text-main);">${this.state.zoom}%</strong></span>
                        <span style="opacity: 0.6;">HISTORY: <strong style="color: var(--text-main);">${this.state.historyIndex + 1} / ${this.state.history.length}</strong></span>
                    </div>
                </footer>
        ...
        return [
            {
                ...
                structure: {
                    ...
                    children: [
                        { type: 'image', props: { src: 'api.php?action=asset&name=hero_placeholder.jpg', borderRadius: 0 } },
                ...
                structure: {
                    ...
                    children: [
                        { type: 'row', props: { gap: 15, alignItems: 'center' }, children: [
                            { type: 'container', props: { width: 60, height: 60, borderRadius: 30, padding: 0 }, children: [{ type: 'image', props: { src: 'api.php?action=asset&name=avatar_placeholder.jpg', borderRadius: 30 } }] },
