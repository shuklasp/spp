...
export function activateWorkflowVisualizer(context: vscode.ExtensionContext) {
    let disposable = vscode.commands.registerCommand('spp.visualizeWorkflow', () => {
        ...
        cp.exec(`php spp.php workflow:dump --file="${tmpPath}" --format=mermaid`, { cwd: workspaceRoot }, (error: any, stdout: string, stderr: string) => {
...
function getWebviewContent(mermaidGraph: string) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
