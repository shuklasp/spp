...
const SCAFFOLDING_COMMANDS: ScaffoldDef[] = [
    // Original 9 commands
...
export function activateScaffolding(context: vscode.ExtensionContext) {
    // Use a shared terminal to avoid spawning dozens
    let scaffoldTerminal: vscode.Terminal | undefined;
    ...
    for (const def of SCAFFOLDING_COMMANDS) {
        context.subscriptions.push(
            vscode.commands.registerCommand(def.command, async (uri: vscode.Uri) => {
