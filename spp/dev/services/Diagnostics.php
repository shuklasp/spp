...
if (!function_exists('live_diagnostics_health')) {
    ...
    {
    $res = \SPP\CLI\CommandManager::execute('dev:diagnostics', ['health', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
...
if (!function_exists('live_list_queue')) {
    ...
    {
    $res = \SPP\CLI\CommandManager::execute('dev:diagnostics', ['list_queue', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
        $data = json_decode($res['output'], true);
        if (isset($data['success']) && !$data['success']) {
            $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
...
if (!function_exists('live_get_event_trace')) {
    ...
    {
    $res = \SPP\CLI\CommandManager::execute('dev:diagnostics', ['get_event_trace', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
        $data = json_decode($res['output'], true);
        if (isset($data['success']) && !$data['success']) {
            $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
...
if (!function_exists('live_get_parikshak_trace')) {
    ...
    {
    $res = \SPP\CLI\CommandManager::execute('dev:diagnostics', ['get_parikshak_trace', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
        $data = json_decode($res['output'], true);
        if (isset($data['success']) && !$data['success']) {
            $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
