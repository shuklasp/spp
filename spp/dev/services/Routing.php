...
{
    $res = \SPP\CLI\CommandManager::execute('dev:routing', ['listpages', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
...
{
    $res = \SPP\CLI\CommandManager::execute('dev:routing', ['savepage', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
        $data = json_decode($res['output'], true);
        if (isset($data['success']) && !$data['success']) {
            $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
...
{
    $res = \SPP\CLI\CommandManager::execute('dev:routing', ['removepage', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
        $data = json_decode($res['output'], true);
        if (isset($data['success']) && !$data['success']) {
            $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
...
{
    $res = \SPP\CLI\CommandManager::execute('dev:routing', ['listservices', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
        $data = json_decode($res['output'], true);
        if (isset($data['success']) && !$data['success']) {
            $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
...
{
    $res = \SPP\CLI\CommandManager::execute('dev:routing', ['saveservice', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
        $data = json_decode($res['output'], true);
        if (isset($data['success']) && !$data['success']) {
            $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
...
{
    $res = \SPP\CLI\CommandManager::execute('dev:routing', ['removeservice', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
        $data = json_decode($res['output'], true);
        if (isset($data['success']) && !$data['success']) {
            $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
