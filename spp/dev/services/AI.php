...
function live_AI_GetRegistry($la, $params) {
    $res = \SPP\CLI\CommandManager::execute('dev:ai', ['getregistry', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
...
function live_AI_Prompt($la, $params) {
    $res = \SPP\CLI\CommandManager::execute('dev:ai', ['prompt', '--payload' => json_encode($params), '--json' => '1']);
    if ($res['success']) {
        $data = json_decode($res['output'], true);
        if (isset($data['success']) && !$data['success']) {
            $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
