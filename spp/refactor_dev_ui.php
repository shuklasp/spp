<?php
// Refactor Script for Dev UI -> CLI Parity

$servicesDir = __DIR__ . '/dev/services';
...
foreach ($files as $file) {
    ...
    $commandName = 'Dev' . $basename . 'Command';
    $cmdIdentifier = 'dev:' . strtolower($basename);
    
    $commandClass = "<?php\nnamespace SPP\\CLI\\Commands;\n\nuse SPP\\CLI\\Command;\n\nclass {$commandName} extends Command\n{\n";
    $commandClass .= "    protected string \$name = '{$cmdIdentifier}';\n";
    $commandClass .= "    protected string \$description = 'Manage Dev {$basename} operations. Usage: {$cmdIdentifier} <action> [--payload=...] [--json]';\n\n";
    ...
    foreach ($functions as $funcName => $data) {
        ...
        $proxyBody .= "            if (!empty(\$data['executeClientCode'])) \$la->executeClientCode(\$data['executeClientCode']);\n";
        $proxyBody .= "            if (!empty(\$data['redirect'])) \$la->redirect(\$data['redirect']);\n";
        $proxyBody .= "        } else {\n";
