    ...
    public function save()
    {
        ...
        if ($result !== false) {
            ...
            if ($rowCount >= $this->maxRowsPerSegment) {
                ...
                @copy($this->filePath, $newSegmentPath);
                $this->segments[] = $newSegmentPath;
                
                // Clear all rows to start fresh, preserving schema and auto-increment ID
                $rows = $this->xpath->query("//row");
                foreach ($rows as $row) {
                    $row->parentNode->removeChild($row);
                }
                
                $tempFile = $this->filePath . '.tmp';
                if ($this->doc->save($tempFile) !== false) {
