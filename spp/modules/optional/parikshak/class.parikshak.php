...
class Parikshak
{
    ...
    private ?string $origDbtype = null;
    private ?string $origSqlitePath = null;
    ...
    public function __construct(?\SPPMod\SPPDB\SPPDB $db = null, ?int $seed = null)
    {
        ...
        $this->origDbtype = \SPP\Module::getConfig('dbtype', 'sppdb');
        $this->origSqlitePath = \SPP\Module::getConfig('sqlite_path', 'sppdb');
        $this->testDbFile = 'spp/scratch/parikshak_' . uniqid() . '.sqlite';
        
        \SPP\Module::setConfig('dbtype', 'sqlite', 'sppdb');
        \SPP\Module::setConfig('sqlite_path', $this->testDbFile, 'sppdb');
    ...
    {
        if ($this->origSqlitePath !== null) {
            \SPP\Module::setConfig('sqlite_path', $this->origSqlitePath, 'sppdb');
        }
        if ($this->origDbtype !== null) {
            \SPP\Module::setConfig('dbtype', $this->origDbtype, 'sppdb');
        }
        if (!empty($this->testDbFile)) {
            $scratchPath = SPP_APP_DIR . '/' . $this->testDbFile;
