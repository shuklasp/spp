...
class ParikshakCommand extends Command
{
    ...
    public function execute(array $args): void
    {
        ...
        $origDbtype = \SPP\Module::getConfig('dbtype', 'sppdb', $appname);
        $origSqlitePath = \SPP\Module::getConfig('sqlite_path', 'sppdb', $appname);

        try {
            // Inject in-memory SQLite for testing to isolate database state
            \SPP\Module::setConfig('dbtype', 'sqlite', 'sppdb', $appname);
            \SPP\Module::setConfig('sqlite_path', ':memory:', 'sppdb', $appname);
            ...
            if ($origDbtype !== null) {
                \SPP\Module::setConfig('dbtype', $origDbtype, 'sppdb', $appname);
            }
            if ($origSqlitePath !== null) {
                \SPP\Module::setConfig('sqlite_path', $origSqlitePath, 'sppdb', $appname);
            }
        }
    }
