...
class RevisionManager
{
    ...
    public static function auditEntity($entity): void
    {
        ...
        try {
            ...
            $db->exec_squery("INSERT INTO %tab% (entity_type, entity_id, delta, created_at, user_id) VALUES (?, ?, ?, ?, ?)",
                $table, [
                    $class,
                    $entity->id,
                    $compressed,
    ...
    public static function getRevision($entity, int $revisionId)
    {
        ...
        $res = $db->exec_squery("SELECT * FROM %tab% WHERE entity_type = ? AND entity_id = ? AND id >= ? ORDER BY id DESC",
            $table, [get_class($entity), $entity->id, $revisionId]
        );
    ...
    public static function getHistory($entity): array
    {
        ...
        return $db->exec_squery("SELECT id, created_at, user_id FROM %tab% WHERE entity_type = ? AND entity_id = ? ORDER BY id DESC",
            $table, [get_class($entity), $entity->id]
        );
    }
}
