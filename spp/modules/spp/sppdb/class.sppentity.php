...
class SPPEntity implements \JsonSerializable, \SPP\Core\EntityInterface
{
    protected $id = null;
    ...
    public function setShardKey(?string $key): self
    {
        $this->shardKey = $key;
        return $this;
    }
    ...
    protected function getDB(): \SPPMod\SPPDB\SPPDB
    {
        return new \SPPMod\SPPDB\SPPDB(null, null, null, null, true, $this->shardKey);
    }

    protected static $_observers = [];
    protected static $_globalScopes = [];

    public static function observe(string $observerClass)
    {
        self::$_observers[static::class][] = $observerClass;
    }

    public static function addGlobalScope(string $name, \Closure $scope)
    {
        self::$_globalScopes[static::class][$name] = $scope;
    }

    public static function getGlobalScopes(): array
    {
        return self::$_globalScopes[static::class] ?? [];
    }
    ...
    protected function fireEvent(string $event)
    {
        ...
        if (isset(self::$_observers[$class])) {
            foreach (self::$_observers[$class] as $observerClass) {
    ...
    protected static function bootTraits()
    {
        ...
        $classLoop = $class;
        while ($classLoop) {
            $traits = array_merge($traits, class_uses($classLoop));
            $classLoop = get_parent_class($classLoop);
        }

        foreach ($traits as $trait) {
            $basename = substr(strrchr('\\' . $trait, '\\'), 1);
    ...
    protected static function loadEntityConfig(string $class)
    {
        ...
        if ($yml_file !== false) {
            ...
            if (isset($ymlData['key_type'])) {
                $config['key_type'] = $ymlData['key_type'];
            }
            if (isset($ymlData['casts']) && is_array($ymlData['casts'])) {
                $config['casts'] = $ymlData['casts'];
            }
    ...
    public function loadFiltered(array $conditions = [], string $sort = null, int $limit = null): array
    {
        ...
        if ($sort) {
            $sortSafe = preg_replace('/[^a-zA-Z0-9_\s\,]/', '', $sort);
            $sql .= ' order by ' . $sortSafe;
        }
    ...
    public function set($attribute, $value)
    {
        $mutatorMethod = 'set' . str_replace(' ', '', ucwords(str_replace('_', ' ', $attribute))) . 'Attribute';
    ...
    public function get($attribute)
    {
        ...
        if (array_key_exists($attribute, $classVar)) {
            ...
            if ($this->currentLanguage !== 'en' && array_key_exists($attribute, $this->_translations)) {
                $value = $this->_translations[$attribute];
                $found = true;
            } else {
                $attributes = $this->getAttributes();
                $dynamicAttributes = self::getMetadata('dynamic_attributes', []);
                if (array_key_exists($attribute, $attributes)) {
                    ...
                    $value = $this->_dynamic_values[$attribute] ?? null;
                    $found = true;
                } elseif (array_key_exists('fields_data', $attributes)) {
                    $fieldsData = $this->_values['fields_data'] ?? '';
                    if (is_string($fieldsData)) {
    ...
    public function save()
    {
        ...
        $this->before_save();
        $evtParams = new \SPP\EventParams(['entity' => $this]);
        \SPP\SPPEvent::fireEvent('entity:before_save', $evtParams);
        ...
        if ($this->id == null) {
            ...
            $this->after_save();
            $evtParams = new \SPP\EventParams(['entity' => $this]);
            \SPP\SPPEvent::fireEvent('entity:after_save', $evtParams);
     ...
     * @param array $entities Array of SPPEntity instances
    ...
    public static function insertMany(array $entities)
    {
        ...
        foreach ($entities as $e) {
            ...
            $e->before_save();
            if (!$e->id) {
                $e->id = $e->createId(); // Generate ID if null
            }
            $record = array_merge([static::getMetadata('id_field') => $e->id], $e->_values);
            // Process casts
            foreach ($record as $k => $v) {
                $record[$k] = $e->uncastAttributeValue($k, $v);
            }
            $records[] = $record;
        }
    ...
    /**
     * Set the DB loaded value, performing JSON casts if needed.
     */
    ...
    public function insert()
    {
        ...
        foreach ($this->_values as $k => $v) {
            if (array_key_exists($k, $attributes)) {
                $val_array[$k] = $this->uncastAttributeValue($k, $v);
            }
        }
        ...
        if ($this->id != null) {
            ...
            if (self::getMetadata('track_revisions')) {
                ...
                if (!empty($delta)) {
                    ...
                    $db->execute_query(
                        "INSERT INTO spp_entity_revisions (entity_class, entity_id, revision_date, author_id, delta_data, log_message) VALUES (?, ?, CURRENT_TIMESTAMP, ?, ?, ?)",
