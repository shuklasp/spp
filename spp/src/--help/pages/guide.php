 ...
 *
 * Generated for application: --help
 *
 * HOW THIS PAGE WORKS:
 *   - This is a "native PHP page" rendered by the SPP view layer
 *   - It is mapped in etc/apps/--help/pages.yml as:
 *       guide:
 *         url: pages/guide.php
...
<div style="max-width: 1100px; margin: 2rem auto; padding: 0 1rem; font-family: Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
    <div style="background: #fff; border-radius: 16px; padding: 2.5rem; box-shadow: 0 4px 20px rgba(0,0,0,0.04); border: 1px solid #e2e8f0;">
        ...
        <p style="color: #94a3b8; font-size: 0.95rem; margin-bottom: 2.5rem;">Application: <code style="background:#f1f5f9;padding:2px 8px;border-radius:6px;">--help</code> &mdash; Everything a complete novice needs to build any workflow</p>
        ...
        <p style="color:#475569; line-height:1.8;">
            ...
            <code>native</code> (raw PHP), <code>api</code> (headless REST), <code>dropin</code> (low-code HTML/PHP). Your app is: <code>--help</code>.
        </p>
        ...
        <p style="color:#475569; line-height:1.8;">Every SPP app has two main directories: <code>etc/apps/--help/</code> for configuration, and <code>src/--help/</code> for source code.</p>
...
└── spp/                           # Framework core (do not modify)
    ...
    └── etc/global-settings.yml    # All apps registered here</pre>
        ...
        <p style="color:#475569; line-height:1.8;">Routes are defined in <code>etc/apps/--help/pages.yml</code>. SPP supports five route types:</p>

        <h3 style="color:#475569; margin-top:1.5rem;">3a. pages.yml Route Types</h3>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
# ── etc/apps/--help/pages.yml ──

defaults:
  home: home                              # Default page when no route matches
  pagedir: /src/--help              # Base directory for page files

pages:
  # TYPE 1: Controller route — calls a PHP class method
  home:
    controller: \App\--help\Serv\HomeController@index
  ...
  api/v1/items/{id}:
    controller: \App\--help\Serv\ApiController@show</pre>
...
// Cache file: var/cache/routes_--help.php</pre>
        ...
        <p style="color:#475569; line-height:1.8;">Controllers live in <code>src/--help/serv/</code>. They handle requests and return either rendered Blade HTML or JSON data.</p>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── src/--help/serv/HomeController.php ──

namespace App\--help\Serv;

class HomeController
{
    // RENDERING A BLADE VIEW:
    // SPPBlade::getInstance() returns the Blade template engine.
    // run('viewName', $data) renders resources/--help/views/viewName.blade.php
    ...
    public function saveAndRedirect()
    {
        ...
        <p style="color:#475569; line-height:1.8;">Blade is the template engine (from Laravel, extended by SPP). Templates live in <code>resources/--help/views/</code>.</p>

        <h3 style="color:#475569; margin-top:1.5rem;">5a. Layout System</h3>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
{{-- ── resources/--help/views/layout.blade.php ── --}}
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
{{-- ── resources/--help/views/home.blade.php ── --}}
...
@endsection</pre>
        ...
        <p style="color:#475569; line-height:1.8;">SPP-UX is the reactive component system. Components are JavaScript classes in <code>src/--help/comp/</code>.</p>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── comp/my-counter.js ──
...
// Themes defined in resources/--help/themes/ with theme.yml</pre>
...
\SPP\SPPEvent::listen('PageNotFound', [new \App\--help\Events\ErrorHandler(), 'onNotFound']);
...
events:
  - event: "AppBoot"
    handler: "App\--help\Events\AppBootHandler::onBoot"
  - event: "PageNotFound"
    handler: "App\--help\Events\ErrorHandler::onNotFound"
  - event: "core.error.exception"
    handler: "App\--help\Events\ErrorHandler::onException"</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
# ── etc/apps/--help/services.yml ──
services:
  - name: "task.create"
    script: "src/--help/serv/task_create.php"
  - name: "task.list"
    script: "src/--help/serv/task_list.php"
...
// ── PHP (src/--help/serv/task_create.php) ──
...
try {
    ...
    return $blade-&gt;run('error', ['message' =&gt; 'Something went wrong']);
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── src/--help/middleware/AuthGuard.php ──
namespace App\--help\Middleware;
...
// ── etc/apps/--help/middleware.yml (global pipeline) ──
middleware:
  global:
    - \App\--help\Middleware\CorsMiddleware
    - \App\--help\Middleware\SessionMiddleware
  groups:
    auth:
      - \App\--help\Middleware\AuthGuard
    api:
      - \App\--help\Middleware\ApiRateLimit

// Per-route middleware (in pages.yml):
// dashboard:
//   controller: \App\--help\Serv\DashboardController@index
//   middleware: [auth]
...
// Each middleware calls $next() to pass to the next one.</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── Creating an Entity ──
// Run: php spp.php make:entity --app=--help Task
// This creates src/--help/entities/Task.php

namespace App\--help\Entities;

class Task extends \SPPMod\SPPDB\Entity
{
    protected string $table = '--help_tasks';  // Uses table prefix
...
// ── Migrations ──
// php spp.php migrate --app=--help
// php spp.php migrate:rollback --app=--help
// php spp.php db:seed --app=--help</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
# ── etc/apps/--help/forms/contact.yml ──
id: contact
...
if ($result['success']) {
    header('Location: /--help/dashboard');
}
...
//   session_save_path: tcp://127.0.0.1:6379</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
// ── Language files: resources/--help/lang/en.json ──
{
...
Container::bind('mailer', function() {
    return new \App\--help\Services\MailService();
});

// ── Singleton (same instance every time) ──
Container::singleton('cache', function() {
    return new \App\--help\Services\CacheService();
});
...
// File naming: test.TaskTest.php in src/--help/tests/</pre>
...
// Also: -&gt;toBeTrue(), -&gt;toBeFalse(), -&gt;toBeNull(), -&gt;toContain()</pre>
        ...
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
php spp.php test:run --app=--help               # Run full test suite
php spp.php test:run --app=--help --coverage    # With code coverage report
php spp.php test:run --app=--help TaskTest      # Run single test class
php spp.php test:blueprint --app=--help          # Auto-generate test stubs
php spp.php test:monkey --app=--help             # Fuzz/monkey testing
php spp.php test:evolve --app=--help             # Evolutionary testing</pre>

        <!-- ================================================================ -->
        <!-- SECTION 19: Themes -->
        <!-- ================================================================ -->
        <h2 style="color:#6366f1; margin-top:2.5rem; border-bottom: 2px solid #e2e8f0; padding-bottom: 0.5rem;">19. Themes</h2>
        <p style="color:#475569; line-height:1.8;">Themes are defined in <code>resources/--help/themes/</code> with a <code>theme.yml</code> config.</p>
        <pre style="background:#0f172a; color:#e2e8f0; padding:1.2rem; border-radius:10px; overflow-x:auto; font-size:0.82rem; line-height:1.7;">
# ── resources/--help/themes/default/theme.yml ──
name: "Default Theme"
version: "1.0"
css:
...
# ── etc/apps/--help/config.yml ── (app-specific)
app:
  name: "--help"
...
$value = \SPP\SPPConfig::get('app.name');       // "--help"
...
// Log rotation is automatic (configurable max file size)</pre>
        ...
        <p style="color:#475569; line-height:1.8;">Follow these steps to build your first feature in <code>--help</code>:</p>
        ...
        <div style="background:#f0fdf4; border-left:4px solid #22c55e; padding:1.2rem 1.5rem; border-radius:0 10px 10px 0; margin:1rem 0;">
            <ol style="color:#475569; line-height:2.2; margin:0; padding-left:1.2rem;">
                <li><b>Explore your app</b> &mdash; Visit <code>/--help/home</code> in your browser</li>
                <li><b>Edit a controller</b> &mdash; Modify <code>src/--help/serv/HomeController.php</code></li>
                <li><b>Edit a Blade view</b> &mdash; Modify <code>resources/--help/views/home.blade.php</code></li>
                <li><b>Create an entity</b> &mdash; Run <code>php spp.php make:entity --app=--help Task</code></li>
                <li><b>Run migrations</b> &mdash; Run <code>php spp.php migrate --app=--help</code></li>
                <li><b>Add a new route</b> &mdash; Edit <code>etc/apps/--help/pages.yml</code></li>
                <li><b>Create a YAML form</b> &mdash; Add a new file in <code>etc/apps/--help/forms/</code></li>
                <li><b>Build a component</b> &mdash; Create a JS file in <code>src/--help/comp/</code></li>
                <li><b>Write tests</b> &mdash; Add tests in <code>src/--help/tests/</code></li>
                <li><b>Run tests</b> &mdash; <code>php spp.php test:run --app=--help</code></li>
        ...
        <div style="margin-top:2rem; padding:1.5rem; background:#eff6ff; border-radius:10px; text-align:center;">
            ...
            <p style="color:#64748b; font-size:0.85rem; margin:0.5rem 0 0;">Remove the <code>'guide'</code> route from <code>etc/apps/--help/pages.yml</code> and delete this file.</p>
        </div>
