...
class DevLegacyCommand extends Command
{
    protected string $name = 'dev:legacy';
    protected string $description = 'Manage Dev Legacy operations. Usage: admin:legacy <action> [--payload=...] [--json]';
...
JS;
    ...
    private function handleGetProfile(array $payload, array $args): void {
        ...
        $this->json(['success' => false, 'error' => "Profile not found.", "error"], $args); return;
        return;
    ...
    private function handleGetFormHtml(array $payload, array $args): void {
        ...
        try {
            $config = \Symfony\Component\Yaml\Yaml::parse($yaml);
            ob_start(); require SPP_APP_DIR . '/spp/admin/partials/generated/devlegacy_1.php'; $html = ob_get_clean();
            if (isset($config['fields']) && is_array($config['fields'])) {
                foreach ($config['fields'] as $key => $f) {
                    ob_start(); require SPP_APP_DIR . '/spp/admin/partials/generated/devlegacy_2.php'; $html .= ob_get_clean();
    ...
    private function handleListModules(array $payload, array $args): void {
        ...
        try {
            ...
            $this->notify("Modules listed.", "success", $args);
