...
class DevAuthCommand extends Command
{
    protected string $name = 'dev:auth';
    protected string $description = 'Manage Dev Auth operations. Usage: admin:auth <action> [--payload=...] [--json]';
    ...
    try {
        ...
        if ($success) {
            ...
            $_SESSION['spp_dev_user'] = $username;
            \SPP\SPPSession::setSessionVar('__sppauth_user__', $username);
            $this->notify("Login successful.", "success", $args);
    ...
    try {
        ...
        if (\SPPMod\SPPAuth\MFA::verifyCode($secret, $code)) {
            ...
            $_SESSION['spp_dev_user'] = $user->username;
    ...
    try {
        ...
        if ($user) {
            ...
            $this->notify("Magic link login successful.", "success", $args);
            $this->json(['user' => $user->username], $args); return;
        } else {
    ...
    require_once SPP_MODULES_DIR . '/spp/sppauth/class.sppauth.php';
    if (isset($_SESSION['spp_dev_fallback'])) {
        unset($_SESSION['spp_dev_fallback']);
    }
    ...
    try {
        ...
        } elseif (isset($_SESSION['spp_dev_user'])) {
            $username = $_SESSION['spp_dev_user'];
        }
