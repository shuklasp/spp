    ...
    public function execute(array $args): void
    {
        ...
        foreach ($args as $arg) {
            ...
            if (str_starts_with($arg, '--extends='))
                $extendsClass = substr($arg, 10);
            if (str_starts_with($arg, '--login='))
                $loginEnabled = strtolower(substr($arg, 8)) === 'true' || substr($arg, 8) === '1';
            if (str_starts_with($arg, '--relations='))
                $relationsArg = substr($arg, 12);
        }
