...
if (!function_exists('live_ai_parse_scaffold')) {
    function live_ai_parse_scaffold($la, $params) {
        ...
        if (!$prompt) return $la->setStatus('error')->notify("Prompt required.", "error");
...
if (!function_exists('live_clone_app')) {
    function live_clone_app($la, $params) {
        ...
        if (!$source || !$target) return $la->setStatus('error')->notify("Source and target required.", "error");
...
if (!function_exists('live_scaffold_template')) {
    function live_scaffold_template($la, $params) {
        ...
        if (!$template) return $la->setStatus('error')->notify("Template name required.", "error");
...
if (!function_exists('live_scaffold_test')) {
    function live_scaffold_test($la, $params) {
        ...
        if (!$entityName) return $la->setStatus('error')->notify("Entity name required.", "error");
...
if (!function_exists('live_restore_revision')) {
    function live_restore_revision($la, $params) {
        ...
        if (!$name || !$timestamp) return $la->setStatus('error')->notify("Name and timestamp required.", "error");
...
if (!function_exists('live_preview_migration')) {
    function live_preview_migration($la, $params) {
        ...
        if (!$name || !$yamlContent) return $la->setStatus('error')->notify("Entity name and YAML required.", "error");

        try {
            $config = \Symfony\Component\Yaml\Yaml::parse($yamlContent);
            $table = $config['table'] ?? '';
            if (!$table) return $la->setStatus('error')->notify("Table name not defined in YAML.", "error");
...
if (!function_exists('live_list_commands')) {
    function live_list_commands($la, $params) {
        ...
        if ($action === 'get_command_ui') {
            ...
            if (!$cmd) return $la->setStatus('error')->notify("Command not found.", "error");
...
if (!function_exists('live_introspect_table')) {
    function live_introspect_table($la, $params) {
        $appContext = $params['appname'] ?? 'default';
        $table = $params['table'] ?? '';
        if (!$table) return $la->setStatus('error')->notify("Table name required.", "error");
        try {
            $db = new \SPPMod\SPPDB\SPPDB();
            ...
            if ($driver === 'mongodb') {
                ...
                if (empty($res)) return $la->setStatus('error')->notify("Collection is empty, cannot infer schema.", "error");
                $doc = (array)$res[0];
                foreach ($doc as $k => $v) {
                    if ($k === '_id') continue;
...
if (!function_exists('live_scaffold_form')) {
    function live_scaffold_form($la, $params) {
        ...
        $config = $params['config'] ?? [];
        if (is_string($config)) $config = json_decode($config, true);
        if (!$entityName || empty($config['attributes'])) return $la->setStatus('error')->notify("Entity configuration required.", "error");
...
if (!function_exists('live_save_form_config')) {
    function live_save_form_config($la, $params) {
        ...
        $name = $params['name'] ?? '';
        $yaml = $params['yaml'] ?? '';
        if (!$name) return $la->setStatus('error')->notify("Form name required.", "error");
