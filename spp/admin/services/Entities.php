...
if (!function_exists('live_Entities_List')) {
    ...
    {
        $res = \SPP\CLI\CommandManager::execute('admin:entities', ['list', '--payload' => json_encode($params), '--json' => '1']);
        if ($res['success']) {
...
if (!function_exists('live_Entities_Save')) {
    ...
    {
        $res = \SPP\CLI\CommandManager::execute('admin:entities', ['save', '--payload' => json_encode($params), '--json' => '1']);
        if ($res['success']) {
            $data = json_decode($res['output'], true);
            if (isset($data['success']) && !$data['success']) {
                $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
...
if (!function_exists('live_Entities_Delete')) {
    ...
    {
        $res = \SPP\CLI\CommandManager::execute('admin:entities', ['delete', '--payload' => json_encode($params), '--json' => '1']);
        if ($res['success']) {
            $data = json_decode($res['output'], true);
            if (isset($data['success']) && !$data['success']) {
                $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
...
if (!function_exists('live_Entities_ParseYAML')) {
    ...
    {
        $res = \SPP\CLI\CommandManager::execute('admin:entities', ['parseyaml', '--payload' => json_encode($params), '--json' => '1']);
        if ($res['success']) {
            $data = json_decode($res['output'], true);
            if (isset($data['success']) && !$data['success']) {
                $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
...
if (!function_exists('live_Entities_DumpYAML')) {
    ...
    {
        $res = \SPP\CLI\CommandManager::execute('admin:entities', ['dumpyaml', '--payload' => json_encode($params), '--json' => '1']);
        if ($res['success']) {
            $data = json_decode($res['output'], true);
            if (isset($data['success']) && !$data['success']) {
                $la->setStatus('error')->notify($data['error'] ?? 'Command failed.');
