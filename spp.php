<?php
/**
 * SPP Framework Scaffolding Engine Console Dispatcher.
 * Natively provisions self-contained application contexts embedding local runtime configurations under the /src directory map.
 * Generates an end-to-end Instructional Skeleton App complete with full SPP branding, asset routes, sample forms, declarative view pages, and components.
 */

if (php_sapi_name() !== 'cli') {
    die("Error: spp.php must be run from the command line interface.\n");
}
...
if ($command === 'create:app') {
    ...
    if (empty($appName) || str_starts_with($appName, '--')) {
        die("Error: Please provide a valid target application identifier. Example: php spp.php create:app dashboard\n");
    }
    ...
    scaffoldApplication($appName, $mode, $aiBlueprint);
    exit(0);
}

if ($command === 'import:component') {
    $compName = $args[1] ?? '';
    if (empty($compName) || str_starts_with($compName, '--')) {
        die("Error: Target component identifier required. Example: php spp.php import:component UI/DataGrid\n");
    }
    $targetApp = 'default';
    foreach ($args as $arg) {
    ...
    $destDir = __DIR__ . '/src/' . $targetApp . '/components/' . dirname($compSafe);
...
if ($command === 'audit:lineage') {
    $targetLog = __DIR__ . '/var/logs/merkle_lineage.log';
    foreach ($args as $arg) {
        if (str_starts_with($arg, '--app=')) {
            $appName = substr($arg, 6);
            $targetLog = __DIR__ . '/src/' . $appName . '/var/logs/merkle_lineage.log';
        }
    }
...
if ($command === 'module:list') {
    ...
    $modsDir = __DIR__ . '/spp/modules/spp';
    if (is_dir($modsDir)) {
...
function showHelp(): void {
    echo "================================================================================\n";
    echo "  SPP Enterprise Scaffolding Engine Console\n";
    echo "================================================================================\n";
    echo "Available Commands:\n";
    echo "  create:app <name> [--mode=spa] [--ai-blueprint=] Scaffolds a self-contained skeleton app natively\n";
    echo "  import:component <name> [--target=]        Imports pristine air-gapped sovereign UI components\n";
...
function scaffoldApplication(string $appName, string $mode, ?string $aiBlueprint = null): void {
    $baseDir = __DIR__ . '/src/' . $appName;
