<?php
// Check which elements have been rewritten (they won't have Wikipedia-style markup)
...
foreach ($all_elements as $sym => $name) {
    $extract = $data[$sym]['extract_html'] ?? '';
    // Rewritten content typically starts with <p><b>ElementName</b> and does NOT have mw-parser-output or infobox classes
    if (strpos($extract, 'mw-parser-output') !== false || strpos($extract, 'infobox') !== false || strpos($extract, 'mw-heading') !== false) {
        $not_rewritten[] = "$sym ($name)";
    } else {
        $rewritten[] = "$sym ($name)";
    }
}
...
echo implode(', ', $rewritten) . "\n\n";
echo "=== NOT YET REWRITTEN (" . count($not_rewritten) . ") ===\n";
echo implode(', ', $not_rewritten) . "\n";
